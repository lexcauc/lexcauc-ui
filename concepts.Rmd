...
display %>% group_by(lc.id,lang) %>% mutate(orthographic = paste(orthographic, collapse="; ")) %>% slice(1) %>% ungroup() %>% group_by(lex.id,lang) %>% mutate(orthographic = paste(orthographic, collapse=", ")) %>% slice(1) %>% ungroup() %>% transmute(id = lex.id, rus = rus, eng = eng, orthographic = orthographic, lang = lang) %>% spread(lang,orthographic) -> display.spread
